...
export default function UserDetail({ params}: any) {
    const hasPermission = true; //Replace with your own permission check login
    let users: IUser[]
    ...
    return (
      <div>
        <h1>User detail</h1>
        <span>User ID: {selectedUser.id?.toString()}</span><br/>
        <span>User Name: </span>
        <span><input type="text" placeholder="User Name" value={selectedUser.name} onChange={(ev)=>{onNameChange(ev)}}/></span><br />
        <span>Gender: </span>
        <span>
          <div>
            <input type="radio" name="gender" id="male-id" placeholder="Gender" value={true} checked={selectedUser.gender} onChange={(ev)=>{onGenderChange(ev)}} />
            <label htmlFor="male-id">Male</label><br />
            <input type="radio" name="gender" id="female-id" placeholder="Gender" value={false} checked={selectedUser.gender} onChange={(ev)=>{onGenderChange(ev)}} />
            <label htmlFor="male-id">Male</label><br />
          </div>
        </span>
        <span>Role: </span>
